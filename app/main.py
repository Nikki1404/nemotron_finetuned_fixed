...
async def run_pcm_through_session(
    ...
    for i in range(0, len(pcm_bytes), chunk_bytes):
        ...
        for ev_type, text, ttfb in events:
            raw_text = clean_model_leaked_tags(text)
            processed = postprocessor.process(raw_text, is_final=(ev_type == "final"))
            text = processed.text
            ...
            event_payload = {
                ...
                "type": ev_type,
                "text": text,
                "raw_text": processed.raw_text if processed.changed else None,
                "corrections": processed.correction_dicts(),
                "language": language,
                "ttfb_ms": ttfb,
            }
