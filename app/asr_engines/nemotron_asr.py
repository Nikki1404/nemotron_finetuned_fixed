...
class NemotronStreamingASR(ASREngine):
    ...
    def __init__(
        ...
        self.max_symbols = int(os.getenv("NEMO_MAX_SYMBOLS", "10"))
    ...
    def stream_transcribe(
        ...
        remaining = available - emitted_frames
        if remaining <= 0:
            return None, cache, prev_hyp, prev_pred_out, emitted_frames, timings

        enough = remaining >= self.shift_frames
...
class StreamingSession:
    ...
    def finalize(self, pad_ms: int) -> str:
        ...
        # A single force-flush step only consumes one encoder shift.  When the
        # client sends EOF immediately after a speech chunk, several shifts can
        # still be pending.  Drain every remaining shift so final words are not
        # truncated.
        t0 = time.perf_counter()
        for _ in range(64):
            before = self.emitted_frames
            text, self.cache, self.prev_hyp, self.prev_pred, self.emitted_frames, t = (
                self.engine.stream_transcribe(
                    audio_f32=self.audio,
                    cache=self.cache,
